...
mass_factor = factor*Mmg24/Msolar*pccm*pccm/2.75/1E3*1E8 #3 is for Mg II 2803; 1E3 is for mA; 1E9 is for display purpose
...
for i in np.arange(y.size):
    print y[i], Sigma_lrg[i], Sigma_lrg_err[i]

#stop
#   Sigma_lrg[i] = Sigma_lrg[i]*2.5/2.
...
print np.log10(A[iAmax_1h_all_left])-8, np.log10(A[iAmax_1h_all_tmp])-8, np.log10(A[iAmax_1h_all_right])-8.
print np.log10(A[iAmax_2h_all_left])-8, np.log10(A[iAmax_2h_all_tmp])-8, np.log10(A[iAmax_2h_all_right])-8.
...
plt.show()
plt.savefig('Halomodel_comparison.eps')
...
plt.show()
plt.savefig('Darkeverything.eps')
...
plt.errorbar(y, Sigma_lrg, yerr=Sigma_lrg_err, capsize=5, fmt='bo', mec='b', ms=10)
plt.text(1.20, 2.0E2, r"\textbf{Mg II (z$\sim$0.5, Zhu et al. 2013)}", color='b', fontsize=14)
plt.xlim(2E-2, 3E1)
plt.ylim(2E-1, 5E2)
plt.xscale('log')
plt.yscale('log')
plt.ylabel(r'$\boldsymbol{\Sigma}_\mathrm{Mg\,II}\ (10^{-8} \mathrm{M}_\odot\ \mathrm{pc^{-2}})$', rotation='vertical', fontsize=24)
...
plt.axhspan(np.min(ratio), np.max(ratio), facecolor='blue', alpha=0.2)
print np.min(ratio), np.max(ratio)
plt.errorbar(y, ratio, yerr=ratio_err, capsize=5, fmt='o', color='b', mec='b', ms=10)
plt.text(1.30, 1.9, r"\textbf{Mg II Gas-to-Mass Ratio}", color='b', fontsize=14)
plt.xlabel(r'$\mathbf{r}_\mathrm{p}$', fontsize=24)
#plt.ylabel(r'$\boldsymbol{\Delta \Sigma}\ (\mathrm{M}_\odot\ \mathrm{pc^{-2}})$', rotation='vertical', fontsize=26)
#plt.ylabel(r'$\log_{10}\ \boldsymbol{f}_\mathrm{Mg\,II}/10^{-8}$', rotation='vertical', fontsize=24)
...
plt.show()
plt.savefig('Darkeverything_Demo.pdf')
plt.savefig('Darkeverything_Demo.eps')
...
np.savetxt('LRG-Sigma.txt', np.column_stack([Sigma_lrg, Sigma_lrg_err]), fmt=['%7.3f','%7.3f'])
np.savetxt('LRG-Sigma_model.txt', np.column_stack([np.log10(RR), Amax_1h*Sigma_1h_max, Amax_2h*Sigma_2h_max, Amax_1h*Sigma_1h_max+Amax_2h*Sigma_2h_max]), fmt=['%10.5f','%10.5f','%10.5f','%10.5f'])
...
plt.setp(ax1.get_xticklabels(), visible=False)
plt.show()
plt.savefig('Halomodel_residuals.eps')
...
plt.show()
plt.savefig('Halomodel_comparison_residuals_extremes.eps')
...
plt.savefig('Chi2_1h_2h_onepanel.eps')
...
plt.axhspan(np.log10(np.min(ratio)), np.log10(np.max(ratio)), facecolor='blue', alpha=0.2)
...
plt.savefig('Chi2_1h_2h_onepanel_dark.pdf')
plt.savefig('Chi2_1h_2h_onepanel_dark.eps')
...
plt.ylabel(r'$\log_{10}\ \boldsymbol{f}_\mathrm{Mg\,II}^\mathrm{1h}/10^{-8}$', rotation='vertical', fontsize=30)
...
plt.plot(np.log10(Mhalo[iMmax_1h]), np.log10(A[iAmax_1h]), 'o', color='yellow', markersize=8)
...
plt.ylabel(r'$\log_{10}\ \boldsymbol{f}_\mathrm{Mg\,II}^\mathrm{2h}/10^{-8}$', rotation='vertical', fontsize=30)
strs =[r'$68.3\%$', r'$95.4\%$', r'$99.7\%$']
fmt = {}
for l,s in zip(cs.levels, strs ): fmt[l] = s
#plt.clabel(cs, cs.levels, fmt=fmt, inline=1, fontsize=10)
plt.plot(np.log10(Mhalo[iMmax_2h]), np.log10(A[iAmax_2h]), 'o', color='yellow', markersize=8)

#plt.plot(np.log10(Mhalo[iMmax_2h]), np.log10(A[iAmax_2h]), 'bx', markersize=15)
plt.setp(ax3.get_xticklabels(), visible=False)
plt.show()
plt.savefig('Chi2_1h_2h.eps')
...
plt.legend([r'\textbf{Halo model}', r'\textbf{1-halo term (gas)}', r'\textbf{2-halo term (gas\&dark matter)}', r'\textbf{1-halo term (dark matter)}'], handlelength=3.4, frameon=False, loc=2)
plt.loglog(RR, vdisp_all_max, 'b', lw=2)
...
plt.xlim(1E-2, 3E1)
plt.ylim(3.0E1, 2.0E3)
plt.show()
plt.savefig('Halomodel_vdisp.eps')
...
M_demo = np.array([11.0, np.log10(Mmax), 15.5])
...
for i in np.arange(ndemo):
    ...
    print iMtmp, iAmax_1h_tmp, iAmax_2h_tmp, nA, nMhalo, Amax_1h_demo[i], Amax_2h_demo[i]
...
plt.xticks([0.1, 1, 10],('100 kpc', '1 Mpc', '10 Mpc'), fontsize=22)
plt.xlabel(r'$\mathbf{r}_\mathrm{p}$', fontsize=24)
plt.ylabel(r'$\boldsymbol{\Sigma}_\mathrm{Mg\,II}^\mathrm{model}\ (10^{-8} \mathrm{M}_\odot\ \mathrm{pc^{-2}})$', rotation='vertical', fontsize=26)
plt.show()
plt.savefig('Halomodel_demo.eps')

plt.figure(figsize=(9,13))
plt.clf()
ax = plt.subplot2grid((ndemo+3,1),(0,0), rowspan=3)
plt.subplots_adjust(left=0.15, bottom=0.15, hspace=0)
plt.text(1.3E0, 19E1, r'$\log_{10} \boldsymbol{\mathrm{M}}_\mathrm{halo}/\boldsymbol{\mathrm{M}}_\odot=$', color='k', fontsize=20)
for i in np.arange(ndemo):
...
plt.xlabel(r'$\mathbf{r}_\mathrm{p}$', fontsize=26)
plt.show()
plt.savefig('Halomodel_demo_bestfit_residuals.eps')
...
plt.xticks([0.1, 1, 10],('100 kpc', '1 Mpc', '10 Mpc'), fontsize=22)
plt.xlabel(r'$\mathbf{r}_\mathrm{p}$', fontsize=26)
plt.savefig('Halomodel_demo_bestfit_residuals_1.eps')

plt.figure(figsize=(9,8))
plt.clf()
plt.subplots_adjust(left=0.15, bottom=0.15, hspace=0)
for i in np.arange(ndemo):
    ax = plt.subplot2grid((ndemo,1),(i,0), rowspan=1)
    plt.errorbar(y, (Sigma_lrg-Sigma_all[:,iAmax_1h_demo[i],iAmax_2h_demo[i],iMmax_demo[i]])/Sigma_all[:,iAmax_1h_demo[i],iAmax_2h_demo[i],iMmax_demo[i]], yerr=Sigma_lrg_err/Sigma_all[:,iAmax_1h_demo[i],iAmax_2h_demo[i],iMmax_demo[i]], capsize=5, fmt='o', mec=outcolor[i], mfc=outcolor[i], ecolor=outcolor[i], ms=10)
    #plt.plot(RR, ((Amax_1h*Sigma_1h_max+Amax_2h*Sigma_2h_max)-(Amax_1h_demo[i]*Sigma_1h_demo[:,i]+Amax_2h_demo[i]*Sigma_2h_demo[:,i])), '--', color=outcolor[i], dashes=dashes[i], lw=4)
    plt.xscale('log')
    plt.plot([1E-2,5E1],[0,0], 'k--', lw=1)
    ...
    #if i != ndemo-1: plt.ylim(-1.49E0, 1.49E0)
    #if i == ndemo-1: plt.ylim(-1.E0, 2.E0)
    if i==0: plt.ylim(-1.01E0, 1.49E0)
    if i==1: plt.ylim(-1.01E0, 1.49E0)
    if i==2: plt.ylim(-1.01E0, 2.69E0)
    plt.setp(ax.get_xticklabels(), visible=False)
    #if i == 0: plt.text(2E0, 1.2E0, r'$\log_{10} \boldsymbol{\mathrm{M}}_\mathrm{halo}/\boldsymbol{\mathrm{M}}_\odot='+'%4.1f' % M_demo[i] +'$', color=outcolor[i], weight='bold', fontsize=20)
    #if i != 0: plt.text(1.7E1, 1.2E0, r'$'+'%4.1f' % M_demo[i] +'$', color=outcolor[i], weight='bold', fontsize=20)
    if i==0: plt.text(5E0, 0.90E0, r'$\boldsymbol{\chi}^2/{dof}='+'%4.2f' % (Chi2[iAmax_1h_demo[i], iAmax_2h_demo[i], iMmax_demo[i]]/14.)+'$', color=outcolor[i], fontsize=20)
    if i==1: plt.text(5E0, 0.90E0, r'$\boldsymbol{\chi}^2/{dof}='+'%4.2f' % (Chi2[iAmax_1h_demo[i], iAmax_2h_demo[i], iMmax_demo[i]]/14.)+'$', color=outcolor[i], fontsize=20)
    if i==2: plt.text(5E0, 2.00E0, r'$\boldsymbol{\chi}^2/{dof}='+'%4.2f' % (Chi2[iAmax_1h_demo[i], iAmax_2h_demo[i], iMmax_demo[i]]/14.)+'$', color=outcolor[i], fontsize=20)
    #if i == 0: plt.text(5E0, 1.0E0, r'$\boldsymbol{\chi}^2/{dof}='+'%4.2f' % (Chi2[iAmax_1h_demo[i], iAmax_2h_demo[i], iMmax_demo[i]]/14.)+'$', fontsize=20)
    #if i != 0: plt.text(1.7E1, -2.0E0, r'$'+'%4.2f' % (Chi2[iAmax_1h_demo[i], iAmax_2h_demo[i], iMmax_demo[i]]/14.)+'$', fontsize=20)
#   plt.text(1E1, 1.5E0, r'$\boldsymbol{\chi}^2/{dof}='+'%4.2f' % (Chi2[iAmax_1h_15, iAmax_2h_15, nMhalo-1]/14.)+'$', fontsize=20)
    if i==ndemo/2: plt.ylabel(r'$\boldsymbol{\delta \Sigma}_\mathrm{Mg\,II}/\boldsymbol{\Sigma}_\mathrm{Mg\,II}^\mathrm{model}$', rotation='vertical', fontsize=26)
...
plt.xlabel(r'$\mathbf{r}_\mathrm{p}$', fontsize=26)
plt.show()
plt.savefig('Halomodel_demo_bestfit_residuals_2.eps')
